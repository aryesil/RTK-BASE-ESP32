...
/* Task configuration */
/* Trimmed to fit an ESP32-WROOM-32D alongside the base station. The defaults
 * are sized for a board with PSRAM: measured here, the DERP task reported
 * 12216 bytes of its 14336 still free, and creating the coordination task
 * failed outright for want of a contiguous block. The coordination task keeps
 * the most headroom because it runs the TLS handshake and the cJSON parse. */
#define ML_TASK_NET_IO_STACK    (5 * 1024)
#define ML_TASK_NET_IO_PRIO     7
#define ML_TASK_NET_IO_CORE     0

/* 8 kB, not 5: the DERP task runs an mbedTLS handshake and 5 kB left it with
 * 1608 bytes free, which showed up as "SSL - Bad input parameters". The room
 * comes from cutting WIREGUARD_MAX_PEERS to 6. */
#define ML_TASK_DERP_TX_STACK   (8 * 1024)
#define ML_TASK_DERP_TX_PRIO    5
#define ML_TASK_DERP_TX_CORE    0

#define ML_TASK_COORD_STACK     (10 * 1024)
#define ML_TASK_COORD_PRIO      5
/* All on core 0, next to lwIP and the TCPIP task.
 *
 * Splitting the client across both cores was tried and measured: the telemetry
 * WebSocket recovered, but the tunnel stopped carrying traffic entirely - the
 * WireGuard and DERP paths depend on lwIP callbacks that run on core 0, and the
 * cross-core hop cost more than it bought. Core 1 stays reserved for the GNSS
 * serial path and the RTCM fan-out, which is the one thing on this board that
 * must not be disturbed. */
#define ML_TASK_COORD_CORE      0

#define ML_TASK_WG_MGR_STACK    (6 * 1024)
...
/* Four, not thirty-two. Each region carries four node entries of 132 bytes,
 * so the table was 17 kB inside microlink_t - a single heap allocation at
 * init, sized for every relay region Tailscale operates worldwide. A node uses
 * its home region and falls back to at most a couple of others, and this board
 * needs the 15 kB far more than it needs the list. */
...
/* The MapResponse buffer is claimed at startup and held; see ml_coord.c. */
esp_err_t ml_coord_alloc_map_buffer(void);
uint8_t  *ml_coord_map_buffer(void);
