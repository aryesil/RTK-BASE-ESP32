...
// ---------------------------------------------------------------------------
// Only the RTCM consumer slots shrink. The per-satellite signal rows stay on
// the frame; they are about a third of it but that is a transient buffer, not
// held memory, and they are what the sky plot and the C/N0 charts are drawn
// from. The ionosphere arcs, which used to be another fifth, are fetched from
// /api/iono only while the Overview page is open, so the monitor runs in both
// profiles.
// History keeps its full twelve hours: its 23 kB ring lives in the IRAM heap,
// which nothing else uses and which the client cannot use either (see
// History.cpp). If that allocation fails in this profile, history is off
// rather than taken from ordinary RAM.
// Kept at full size: every RTCM path, the base configuration, the network
// settings, the rover back channel, history, the ionospheric monitor, the
// sky plot and the headline status.
#define LEAN_HISTORY_SAMPLES HISTORY_SAMPLES
